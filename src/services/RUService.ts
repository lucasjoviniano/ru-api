...
class RUService {
    ...
    private getMeals(html: string, separator: string = ':') {
        ...
        const labels = element.toArray()
            .map(str => { return $(str).text().trim()})
            .filter(value => !(value.includes("RU - I") || value.includes("RU - II") || value.includes("RU III")))
            .map(str => str.replace(';', ""))
        ...
        const matches = labels.map((value) => {
            ...
            } else if (value.includes("Almoco") || value.includes("Jantar") || value.includes("Almoço")) {
                indexes.push(labels.indexOf(value))
